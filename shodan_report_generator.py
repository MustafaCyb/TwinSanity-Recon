#!/usr/bin/env python3
"""
shodan_report_generator.py - Creates a clean, interactive, dark-themed HTML report 
from the aggregated analysis of Shodan scan results.

It can generate two types of reports:
1. A full report with LLM-powered analysis and summaries.
2. A raw data report with only the comprehensive, searchable vulnerability list if
   no LLM analysis is provided.
"""
...
<head>
    ...
    <title>Twinsanty Security Analysis Report</title>
    <style>
        :root {{
        ...
        .stat-label {{ color: var(--muted-text); font-size: 0.9rem; }}
...
<div class='container'>
    <div class='header'>
        <h1>Twinsanty Security Analysis Report</h1>
    ...
    <div class='card grid'>
        ...
        <div class='stat-box'><div class='stat-number'>{len(high_risk_assets) if is_llm_run else 'N/A'}</div><div class='stat-label'>LLM-Flagged Assets</div></div>
        <div class='stat-box'><div class='stat-number'>{total_cves_found}</div><div class='stat-label'>Total Unique CVEs Found</div></div>
        <div class='stat-box'><div class='stat-number'>{len(recommended_actions) if is_llm_run else 'N/A'}</div><div class='stat-label'>LLM Actions Recommended</div></div>
    </div>
    ...
    <div class='footer'><p>Report generated by the Twinsanty Security Analysis Agent.</p></div>
</div>
