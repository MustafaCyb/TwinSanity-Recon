...
import subprocess
from legacy_report_generator import generate_html_report
from urllib3.exceptions import ProtocolError
from http.client import RemoteDisconnected

# ---------- Configuration ----------
CVE_CACHE_FILENAME = "cve_cache.json"
CVE_CACHE_TTL_SECONDS = 3 * 24 * 60 * 60  # 3 days
DEFAULT_UA = "TwinSanity-Recon/2.0"
CVE_REQUEST_DELAY = 1.5  
...
def load_config_keys():
    try:
        with open("config.yaml", "r", encoding='utf-8') as f:
            ...
            keys = c.get('api_keys', {})
            # Set env vars for compatibility with rest of script
            if keys.get('shodan'):
                os.environ['SHODAN_API_KEY'] = keys['shodan']
            if keys.get('nvd'):
                os.environ['NVD_API_KEY'] = keys['nvd']
            return keys
...
def create_session(retries: int = 5, backoff: float = 1.0, ua: str = DEFAULT_UA,
...
def fetch_cve_from_circl(session: requests.Session, cve: str, timeout: int = 15, proxies: Optional[Dict] = None, max_retries: int = 3) -> Optional[Dict]:
    """Fetch CVE from CIRCL with retry logic for connection drops."""
    url = f"https://cve.circl.lu/api/cve/{cve}"
    
    for attempt in range(max_retries):
        try:
            # Add delay before request to prevent rate limiting
            time.sleep(CVE_REQUEST_DELAY * (attempt + 1))
            ...
            if attempt < max_retries - 1:
                logging.debug("CIRCL connection dropped for %s, retry %d/%d", cve, attempt + 1, max_retries)
                time.sleep(1.0 * (attempt + 1))  # Increased exponential backoff
                continue
            logging.debug("Failed to fetch CVE %s from CIRCL after %d retries: %s", cve, max_retries, e)
        except Exception as e:
...
# Semaphore and Lock for thread-safe operations
cve_global_semaphore: Optional[threading.Semaphore] = None
cve_cache_lock: Optional[threading.Lock] = None
...
def fetch_cve_details(session: requests.Session, cve: str, cache: Dict[str, Dict], 
    ...
    """Fetch CVE details from specified sources with thread-safe cache."""
    global cve_global_semaphore, cve_cache_lock
    ...
    if cve_cache_lock:
        with cve_cache_lock:
            if cve in cache:
                ...
                if fetched_at and (time.time() - fetched_at) < CVE_CACHE_TTL_SECONDS and cached.get("summary"):
                    return cached.copy()  # Return copy to avoid modification issues
    else:
        # Fallback without lock (shouldn't happen in normal operation)
        if cve in cache:
            cached = cache[cve]
            fetched_at = cached.get("fetched_at")
            if fetched_at and (time.time() - fetched_at) < CVE_CACHE_TTL_SECONDS and cached.get("summary"):
    ...
    try:
        # Try sources in specified order - prefer sources that return CVSS scores
        best_result = None
        for source in cve_sources:
            ...
            if res and res.get("summary"):
                # If we have CVSS score, use this result immediately
                cvss_val = res.get("cvss") or res.get("cvss3")
                if cvss_val and float(cvss_val) > 0:
                    ...
                    return res
                # Store as fallback if no CVSS but has summary
                if best_result is None:
                    best_result = res
                    best_result["source"] = source
        
        # Use best available result (even without CVSS)
        if best_result:
            best_result["fetched_at"] = time.time()
            if cve_cache_lock:
                with cve_cache_lock:
                    cache[cve] = best_result
            else:
                cache[cve] = best_result
            time.sleep(polite_sleep)
            return best_result
        ...
        out = {"id": cve, "summary": None, "cvss": None, "cvss3": None, "references": None, "last_modified": None, "fetched_at": time.time()}
...
def main():
    ...
    parser.add_argument("--proxies-file","-P", type=str, help="optional file with proxy URLs (one per line), e.g. http://127.0.0.1:8080, (HTTP Proxies only vailed)")
    ...
    parser.add_argument("--max-cve-workers-global","-MCWGB", type=int, default=4, help="max concurrent CVE fetches global (default: 4 to prevent rate limiting)")
    parser.add_argument("--max-cve-workers-per-ip","-MCWIP", type=int, default=2, help="max concurrent CVE fetches per-IP (default: 2)")
    ...
    parser.add_argument("--input-as-domains", action="store_true", help="Treat each line in the input file as a domain for discovery.")
    ...
    if args.domain:
        domains_to_scan.add(args.domain)

    # If --input-as-domains is used, read domains from the input file
    if args.input and args.input_as_domains:
        if not Path(args.input).exists():
            logging.error("Input file not found: %s", args.input)
            sys.exit(1)
        with open(args.input, encoding='utf-8', errors='ignore') as f:
    ...
    for domain in domains_to_scan:
        logging.info("Passive enumeration for domain: %s", domain)
        discovered_subs.update(crtsh_passive(domain, session))
        if args.bruteforce:
        ...
        discovered_subs.add(domain) # Ensure the root domain is included

    logging.info("Total unique subdomains discovered: %d", len(discovered_subs))
    ...
    if not all_hosts and not input_ip_map:
        logging.error("No hosts or IPs to process. Provide domains via --domain or --input with --input-as-domains.")
        sys.exit(1)

    # resolve hosts
    hosts_list = sorted(all_hosts)
    total_discovered = len(hosts_list)
    logging.info("Resolving %d hosts (concurrency=%d)", total_discovered, args.concurrency)
    resolved = resolve_hosts_concurrent(hosts_list, concurrency=args.concurrency)

    # build ip->hosts map and track resolution stats
    ip_map: Dict[str, List[str]] = {}
    successfully_resolved = set()
    for h, ips in resolved.items():
        if ips:  # Only count as resolved if we got IPs
            successfully_resolved.add(h)
            for ip in ips:
                ip_map.setdefault(ip, [])
                if h not in ip_map[ip]:
                    ip_map[ip].append(h)

    # Track unresolved hostnames
    unresolved_hosts = all_hosts - successfully_resolved
    
    #Save unresolved hosts to file for reference
    if unresolved_hosts:
        unresolved_file = outdir / "unresolved_subdomains.txt"
        try:
            unresolved_file.write_text("\n".join(sorted(unresolved_hosts)), encoding='utf-8')
            logging.info("Saved %d unresolved subdomains to: %s", len(unresolved_hosts), unresolved_file)
        except Exception as e:
            logging.debug("Failed to save unresolved subdomains: %s", e)

    # Log detailed resolution stats
    logging.info(
        "Subdomain resolution summary: %d discovered, %d resolved (%d IPs), %d offline/unresolved",
        total_discovered,
        len(successfully_resolved),
        len(ip_map),
    ...
    # prepare CVE cache and global semaphore + lock
    ...
    cve_cache_lock = threading.Lock()  # Thread-safe cache operations
    ...
    try:
        ...
        logging.debug(f"Failed to write CSV summary: {type(e).__name__}: {e}")

    # save cve cache
    try:
        save_cve_cache(cache_path, cve_cache)
    except Exception as e:
        logging.debug(f"Failed to save CVE cache: {type(e).__name__}: {e}")


    if args.run_agent:
        # SECURITY FIX (CRIT-5 & HIGH-7): Improved agent handling with proper error reporting
        try:
            ...
            if agent_script.exists():
                ...
                # HIGH-7 FIX: Capture output and check return code
                result = subprocess.run(cmd, check=False, capture_output=True, text=True)
                if result.returncode != 0:
                    logging.error(f"Agent script failed with code {result.returncode}")
                    if result.stderr:
                        logging.error(f"STDERR: {result.stderr[:500]}")
                else:
                    logging.info("Agent analysis completed successfully")
                    if result.stdout:
                        logging.debug(f"Agent output: {result.stdout[:200]}")
            else:
                # CRIT-5 FIX: Option B - Use inline LLM analysis instead
                logging.warning("agent.py not found. Using inline LLM analysis instead.")
                try:
                    ...
                    success, msg = reader.load_results_file(str(combined_file))
                    if success:
                        context = reader.build_context_for_llm()
                        logging.info(f"Built LLM context: {len(context)} chars")
                        
                        # Try local Ollama for analysis
                        try:
                            import ollama
                            response = ollama.chat(
                                model="llama3.1:latest",
                                messages=[{"role": "user", "content": f"Analyze this security scan and provide a summary of key findings:\n\n{context[:8000]}"}]
                            )
                            analysis_path = outdir / "llm_analysis.json"
                            with open(analysis_path, "w", encoding="utf-8") as f:
                                json.dump({"analysis": response["message"]["content"]}, f, indent=2)
                            logging.info(f"LLM analysis saved to {analysis_path}")
                        except ImportError:
                            logging.warning("Ollama not installed, skipping LLM analysis")
                        except Exception as e:
                            logging.warning(f"LLM analysis failed: {e}")
                    else:
                        logging.error(f"Failed to load results for LLM: {msg}")
                except ImportError as ie:
                    logging.warning(f"llm_results_reader not available: {ie}")
                    logging.error("llm_results_reader module not found, cannot run inline analysis")
        except Exception as e:
            logging.error(f"Failed to run agent analysis: {type(e).__name__}: {e}")
