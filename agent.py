...
logging.basicConfig(level=logging.DEBUG, format="%(asctime)s [%(levelname)s] %(message)s",
                    handlers=[logging.FileHandler("shodan_agent.log")])
...
parser.add_argument("--ollama-host", default="http://localhost:11434", help="Ollama server host")
...
if not OLLAMA_API_KEY:
    ...
    try:
        cloud_client = OllamaClient(host="https://api.ollama.com", headers={"Authorization": f"Bearer {OLLAMA_API_KEY}"})
...
def main():
    ...
    results = []
    # ... (the rest of the loop for processing chunks remains the same) ...
    ...
    if not args.no_report:
        try:
            ...
            logger.error("Could not import 'generate_html_report'. Ensure shodan_report_generator.py is present.")
        except Exception as e:
